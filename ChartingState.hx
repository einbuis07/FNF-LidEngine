...
class ChartingState extends MusicBeatState
{
	...
	var lidGroup:FlxUIGroup;
	var noteCounterText:FlxText;
	var spamAmountInput:FlxInputText;
	var expoStrengthInput:FlxInputText;
	...
	**/
	...
	function createNoteCounter():Void
	{
		if (noteCounterText != null) return; 

		noteCounterText = new FlxText(FlxG.width - 350, 15, 300, "Notes: 0", 20); 
		noteCounterText.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		noteCounterText.scrollFactor.set();
		add(noteCounterText);
	}
	...
	function updateNoteCount():Void
	{
		...
		if (_song != null && _song.notes != null) {
			for (i in 0..._song.notes.length) {
				count += _song.notes[i].sectionNotes.length;
			}
		}
		if (noteCounterText != null) noteCounterText.text = "Notes: " + count;
	}

	function snapSectionNotes():Void {
		var stepCrochet:Float = Conductor.stepCrochet;
		if (_song.notes[curSec] != null) {
			for (i in 0..._song.notes[curSec].sectionNotes.length) {
				var note = _song.notes[curSec].sectionNotes[i];
				// Wrap the whole math block in Std.int()
				note[0] = Std.int(Math.floor((note[0] / stepCrochet) + 0.5) * stepCrochet);
			}
		}
		updateGrid();
		updateNoteCount();
	...
	function addLidUI():Void {
		if (UI_box == null) return;
		lidGroup = new FlxUIGroup(10, 10);
		lidGroup.name = 'Lid Tools';

		// --- CATEGORY: SPAM GENERATORS ---
		var labelSpamCat = new FlxText(10, 10, 150, "--- GENERATORS ---");
		labelSpamCat.color = 0xFFFFFF00; 
		lidGroup.add(labelSpamCat);

		var labelAmt = new FlxText(10, 30, 100, "Amt:");
		lidGroup.add(labelAmt);
		
		// 1. CREATE IT FIRST
		spamAmountInput = new FlxInputText(45, 30, 100, '1', 8); 
		
		// 2. NOW SET THE SETTINGS
		spamAmountInput.focusGained = () -> {
			FlxG.stage.focus = null; 
			FlxG.keys.enabled = false; 
		};
		spamAmountInput.focusLost = () -> {
			FlxG.keys.enabled = true; 
		};
		lidGroup.add(spamAmountInput);

		var btnChaos = new FlxButton(10, 50, "Random Chaos", function() {
			var amt = Std.parseInt(spamAmountInput.text);
			if (Math.isNaN(amt)) amt = 1;
			chaosSpam(amt);
		});
		lidGroup.add(btnChaos);

		var btnStair = new FlxButton(10, 75, "Staircase", function() { generateStaircase(); });
		lidGroup.add(btnStair);

		var btnBambi = new FlxButton(10, 100, "Bambi Spam", function() { generateBambiSpam(); });
		lidGroup.add(btnBambi);

		var labelExpoAmt = new FlxText(10, 150, 100, "Expo Strength:");
    	lidGroup.add(labelExpoAmt);

		var btnQuake = new FlxButton(10, 280, "Note-Quake", function() { noteQuake(); });
		lidGroup.add(btnQuake);

		var btnShuffle = new FlxButton(10, 305, "Lane-Shuffle", function() { shuffleLanes(); });
		lidGroup.add(btnShuffle);

		// PASTE YOUR CODE HERE:
		expoStrengthInput = new FlxInputText(110, 150, 40, '2', 8); 
		expoStrengthInput.focusGained = () -> { FlxG.keys.enabled = false; };
		expoStrengthInput.focusLost = () -> { FlxG.keys.enabled = true; };
		lidGroup.add(expoStrengthInput);

		var btnExpo = new FlxButton(10, 175, "Exponential", function() {
			var strength:Float = Std.parseFloat(expoStrengthInput.text);
			if (Math.isNaN(strength) || strength <= 1) strength = 2.0;
			generateExponential(strength);
		});
		lidGroup.add(btnExpo);

		// --- CATEGORY: UTILITY / CLEANUP ---
		var labelUtilCat = new FlxText(10, 210, 150, "--- UTILITY ---");
		labelUtilCat.setFormat(null, 12, 0xFF00FFFF, LEFT, FlxTextBorderStyle.OUTLINE, 0xFF000000);
		lidGroup.add(labelUtilCat);

		var btnJack = new FlxButton(10, 125, "Jack-Hammer", function() {
        	generateJackHammer();
		});
		lidGroup.add(btnJack);

		var btnSnap = new FlxButton(10, 230, "Snap to Grid", function() { snapSectionNotes(); });
		lidGroup.add(btnSnap);

		// Add a button to mirror notes to the opponent's side
		var btnMirror = new FlxButton(10, 250, "Mirror Section", function() {
		var notesToMirror:Array<Array<Dynamic>> = [];
		// 1. Copy the current notes to a temporary list
		for (note in _song.notes[curSec].sectionNotes) {
			notesToMirror.push([note[0], note[1], note[2]]);
		}
		// 2. Add the mirrored versions from that temporary list
		for (note in notesToMirror) {
			var newLane:Int = Std.int((note[1] + 4) % 8);
			_song.notes[curSec].sectionNotes.push([note[0], newLane, note[2]]);
		}
		updateGrid();
		updateNoteCount();
	});
		lidGroup.add(btnMirror);

		UI_box.addGroup(lidGroup);
	}

	function noteQuake():Void {
		if (_song.notes[curSec] != null) {
			for (note in _song.notes[curSec].sectionNotes) {
				// Shakes the note time by up to 20ms in either direction
				note[0] += FlxG.random.float(-20, 20);
			}
		}
		updateGrid();
	}

	function shuffleLanes():Void {
		if (_song.notes[curSec] != null) {
			for (note in _song.notes[curSec].sectionNotes) {
				note[1] = FlxG.random.int(0, 7);
			}
		}
		updateGrid();
	}

	function generateExponential(strength:Float):Void {
		var step:Float = Conductor.stepCrochet;
		var startTime:Float = curSec * 16 * step;
		
		for (i in 0...16) {
			var noteTime:Float = startTime + (i * step);
			
			// This math makes the number of notes grow faster based on your input
			// If Strength is 2, it doubles. If it's 5, it goes crazy!
			var amount:Int = Std.int(Math.pow(strength, i / 4)); 
			
			// Cap it at 100 per step so your PC doesn't explode!
			if (amount > 100) amount = 100; 

			for (j in 0...amount) {
				// Randomly spread across all 8 lanes (BF and Dad)
				_song.notes[curSec].sectionNotes.push([noteTime, FlxG.random.int(0, 7), 0]);
			}
		}
		updateGrid();
		updateNoteCount();
	...
	function generateJackHammer():Void {
		var step:Float = Conductor.stepCrochet;
		var startTime:Float = curSec * 16 * step;
		
		// Pick ONE random lane (0, 1, 2, or 3) for the whole section
		var randomLane:Int = FlxG.random.int(0, 3);
		
		for (i in 0...16) {
			var noteTime:Float = startTime + (i * step);
			// note[0] is time, note[1] is lane, note[2] is length
			_song.notes[curSec].sectionNotes.push([noteTime, randomLane, 0]);
		}
		updateGrid();
		updateNoteCount();
	}

	function chaosSpam(amount:Int):Void {
		for (i in 0...amount) {
			var randomTime:Float = (curSec * 16 * Conductor.stepCrochet) + (FlxG.random.float(0, 15) * Conductor.stepCrochet);
			_song.notes[curSec].sectionNotes.push([randomTime, FlxG.random.int(0, 7), 0]);
		}
		updateGrid();
		updateNoteCount();
	}

	function generateStaircase():Void {
		var step:Float = Conductor.stepCrochet;
		var startTime:Float = curSec * 16 * step;
		for (i in 0...16) {
			var noteTime:Float = startTime + (i * step);
			// We use Std.int for the lane (i % 4) to keep Haxe happy
			_song.notes[curSec].sectionNotes.push([noteTime, Std.int(i % 4), 0]);
		}
		updateGrid();
		updateNoteCount();
	}

	function generateBambiSpam():Void {
		var step:Float = Conductor.stepCrochet;
		var startTime:Float = curSec * 16 * step;
		
		for (i in 0...64) { // Increased from 16 to 64 for density
			var noteTime:Float = startTime + (i * (step / 4)); // Quarter-step timing for vibration
			
			// Randomly place 3 to 6 notes at the EXACT same time
			for (j in 0...FlxG.random.int(3, 6)) {
				var lane:Int = FlxG.random.int(0, 7); // Uses both BF and Dad sides
				_song.notes[curSec].sectionNotes.push([noteTime, lane, 0]);
			}
		}
		updateGrid();
		updateNoteCount();
	...
	override function create()
	{
		...
		var tabs = [
			...
			{name: 'Lid Tools', label: 'Lid Tools'} // NO FUCKIGN WAY THAT WE HAVE THE FUCKING TOOLS..... I BEEN SUFFERING ON THIS SHIT CODE FOR A LONG LONG TIME
		];
